...
export const refresh = async (req, res) => {
   try {
      ...
      const newRefreshToken = jwt.sign({ id: payload.id, role: payload.role }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
export const resendOtp = async (req, res) => {
   try {
      ...
      if (existingOtp) {
         ...
         if (diffSeconds < 60) {
            return res.status(429).json({
               message: `Tunggu ${Math.ceil(60 - diffSeconds)} detik sebelum meminta OTP baru`,
         ...
         if (existingOtp.request_count >= 5) {
            return res.status(429).json({
               message: "Terlalu banyak permintaan OTP. Silahkan coba lagi dalam 15 menit.",
               wait: 900,
            });
         }
         ...
         try {
            ...
            return res.status(500).json({
               message: "Gagal mengirim OTP. Silahkan coba beberapa saat lagi.",
            });
         }
      } else {
         return res.status(400).json({
            message: "OTP belum dibuat. Silahkan login terlebih dahulu.",
         });
      }
      ...
      console.error("Error di resendOtp:", e);
      return res.status(500).json({
...
export const verifyOtp = async (req, res) => {
   try {
      ...
      if (!otp) {
         return res.status(400).json({
            message: "Kode OTP salah. Silahkan periksa kembali.",
         });
      }
      ...
      if (new Date() > otp.expired_at) {
         ...
         return res.status(400).json({
            message: "Kode OTP telah kadaluarsa. Silahkan minta kode baru.",
         });
      }
      ...
      console.error("Error di verifyOtp:", e);
      return res.status(500).json({
         message: "Terjadi kesalahan pada server. Silahkan hubungi administrator.",
      });
   }
   ...
   try {
      ...
      if (user.status !== "verified") {
         ...
         return res.status(403).json({
            message: "Akun Anda belum diverifikasi. Hubungi administrator.",
         });
      }
      ...
      if (trustedDevice) {
         const now = new Date();
         const lastUsed = trustedDevice.last_used ? new Date(trustedDevice.last_used) : new Date(0);

...
export default function PeralatanPage() {
  ...
  const executeDelete = async () => {
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || "Gagal menghapus data alat.");
    }
  };
  ...
  const confirmBulkDelete = (deleteAll = false) => {
    ...
    setDeleteModal({
      ...
      id: null,
      itemName: deleteAll
        ? `semua alat (${data.length} data)`
        : `${selectedDeleteIds.length} alat terpilih`,
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 max-w-xl">
              Kelola daftar peralatan, satuan, dan biaya sewanya khusus untuk project ini.
            </p>
          </div>
        </div>
                ...
                <tr>
                  <td colSpan="6" className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <p className="text-base font-semibold text-gray-600">Alat Kosong</p>
                      <p className="text-sm mt-1 text-gray-500">Belum ada alat yang ditambahkan atau ditemukan.</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      <AnimatePresence>
        {showBulkModal && (
          <m.div transition={{ duration: 0.1 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <m.div transition={{ duration: 0.3 }} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-3xl rounded-md">

              {/* HEADER */}
              <div className="p-5 border-b flex justify-between border-muted-gray">
                <div className="size-4"></div>
                <h2 className="text-lg font-semibold">Import Peralatan dari Master</h2>
                <button className="hover:text-secondary transition-all cursor-pointer" onClick={() => setShowBulkModal(false)}>✕</button>
              </div>
              ...
              <div className="p-5 border-t border-muted-gray flex justify-end gap-3">
                <button
                  ...
                  className="px-5 py-2.5 bg-white border border-gray-200 hover:bg-gray-100 transition-all cursor-pointer font-semibold rounded-md"
                >
                  Batal
                </button>
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-secondary hover:bg-secondary/90 transition-all cursor-pointer font-semibold text-white rounded-md"
                >
                  Import
                </button>
              </div>
      ...
      <AnimatePresence>
        ...
        {showModal && (
          <m.div transition={{ duration: 0.1 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 p-4">
            <m.div transition={{ duration: 0.3 }} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-xl rounded-md overflow-hidden ">
              <div className="flex justify-between items-center p-6 border-b border-gray-100">
                <div className="size-6"></div>
                <h2 className="text-xl font-semibold text-primary flex items-center gap-3">

                  {isEdit ? "Edit Peralatan" : "Tambah Peralatan"}
                </h2>
                <button onClick={closeFormModal} className="text-gray-400 hover:text-secondary hover:bg-gray-100 p-2 rounded-xl transition-all">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6">
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Nama Alat</label>
                    <input
                      type="text"
                      name="nama"
                      placeholder="Contoh: Excavator / Concrete Mixer"
                      value={form.nama}
                ...
                <div className="mt-8 flex justify-end gap-3 pt-5 border-t border-gray-100">
                  <button type="button" onClick={closeFormModal} className="px-5 py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md font-semibold transition-all active:scale-95">
                    Batal
                  </button>
                  <button type="submit" className="px-5 py-2.5 text-white bg-secondary hover:bg-[#e64a0f] rounded-md font-semibold transition-all active:scale-95">
